...
public class MAP {
	...
	public MAP(ArrayList<char[][]> trainingImages, int [] trainingLabels, ArrayList<char[][]> testImages, int [] testLabels, int rows, int columns, int numClasses){
		...
		likelihoods = new double[3][numClasses][rows][columns];
		...
		for (int imageNum = 0; imageNum < numTrainingImages; imageNum++){
			...
			for (int i = 0; i < rows; i++){
				for (int j = 0; j < columns; j++){
					if (currentImage[i][j] == '#'){
						likelihoods[0][digit][i][j]++;
					}
					if (currentImage[i][j] == '+'){
						likelihoods[1][digit][i][j]++;
					}
					else
						likelihoods[2][digit][i][j]++;
				}
			}
		}
		
		int V = 3;	// for Laplace Smoothing
		for (int digit = 0; digit < numClasses; digit++){
			for (int i = 0; i < rows; i++){
				for (int j = 0; j < columns; j++){
					likelihoods[0][digit][i][j] = (likelihoods[0][digit][i][j] + k) / (numTrainingDigits[digit] + k * V);
					likelihoods[1][digit][i][j] = (likelihoods[1][digit][i][j] + k) / (numTrainingDigits[digit] + k * V);
					likelihoods[2][digit][i][j] = (likelihoods[2][digit][i][j] + k) / (numTrainingDigits[digit] + k * V);
				}
			}
		}
	}
		...
		for (int image = 0; image < numTestImages; image++){
			...
			for (int digit = 0; digit < numClasses; digit++){
				...
				for (int i = 0; i < rows; i++){
					for (int j = 0; j < columns; j++){
						if (currentImage[i][j] == '#')
							likelihood = likelihoods[0][digit][i][j];
						else if (currentImage[i][j] == '+')
							likelihood = likelihoods[1][digit][i][j];
						else
							likelihood = likelihoods[2][digit][i][j];
